...
public class Engine {
	...
	public static void main(String[] args) {
		...
		Grade[] grades = new Grade[3]; // 오버로딩으로 바꾸기 위해, 값을 저장하기 위해 배열 시작
		Member[] joins = new Member[3];
		...
		while (true) {
			...
			case 3: //순위는 배열
				...
				if(first > second && first > third) {
					...
					rank1 = 1; rank3 = 2; rank2 = 3;
				}else if(second > first && first > third) {
					rank2 = 1; rank1 = 2; rank3 = 3;
				}else if(second > third && third > first) {
					rank2 = 1; rank3 = 2; rank1 = 3;
				} else if(third > first && first > second) {
					rank3 = 1; rank1 = 2; rank2 = 3;
				}else if(third > second && second > first) {
					rank3 = 1; rank2 = 2; rank1 = 3;
				}else {
					System.out.println("잘못 입력된 값입니다.");
				}
				System.out.println(String.format(grades[0].getName()+" : "+rank1+" , "+grades[1].getName()+" : "+rank2+" , "+grades[2].getName()+" : "+rank3));
				break;
			case 4 : //멤버 3명 가입
				for (int i=0; i<3; ++i) {
					joins[i] = join(sc);
				...
				for (int i=0; i<3; ++i) {
					Member member = joins[i];
					System.out.println(String.format("아이디 : %s, 비밀번호 : %s, 성함 : %s, 나이 : %d", member.getUserid(), member.getPassword(), member.getName(), member.getAge()));
				}
				...
				if(first1 > second1 && first1 > third1) {
					rank4 = 1; rank5 =2; rank6 =3;
				}else if(first1 > third1 && third1 > second1) {
					rank4 = 1; rank6 = 2; rank5 = 3;
				}else if(second1 > first1 && first1 > third1) {
					rank5 = 1; rank4 = 2; rank6 = 3;
				}else if(second1 > third1 && third1 > first1) {
					rank5 = 1; rank6 = 2; rank4 = 3;
				} else if(third1 > first1 && first1 > second1) {
					rank6 = 1; rank4 = 2; rank5 = 3;
				}else if(third1 > second1 && second1 > first1) {
					rank6 = 1; rank5 = 2; rank4 = 3;
				}else {
					System.out.println("잘못 입력된 값입니다.");
				}
				System.out.println("나이순위 - "+joins[0].getName()+" : "+rank4+", "+joins[1].getName()+" : "+rank5+", "+joins[2].getName()+" : "+rank6);
				break;
			}
		}
